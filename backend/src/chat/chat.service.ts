 ...
 * Calls OrchestratorService for real AI responses (Step 3+).
 */
@Injectable()
export class ChatService {
  ...
  async processMessage(dto: SendMessageDto): Promise<ChatResponse> {
    ...
    // 3. Run full Orchestrator pipeline (RAG → ContentAgent → PedagogyAgent)
    let aiResponse: string;
    let orchSources: Record<string, any>[] = [];
    try {
